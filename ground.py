...
E = 0.01 # Trial Eigen value (Enter any small trial value slightly greater than zero for ground state.)
k2 = g2*(E-v) # Initialising k-square.
l2 = (1.0/(N-1))**2 # Initialising l-square.

def wavefunction(E,k2,N):
    Psi[0] = 0 # Initialising starting two values.
    Psi[1] = 0.004443 # These two values should already be given for the normalised wave function.
...
plt.xlabel('x') 
plt.ylabel('Psi(x)') 
plt.title('Ground state , E1 = '+ str(E))
